...
void tst_msg_adv(u16 cluster_id, u16 attrID, u8 * pdata){
	if(cluster_id == 0x405) {
		measured_data.humi = pdata[0] | (pdata[1] << 8);
		measured_data.count++;
	}
	else if(cluster_id == 0x402) {
		measured_data.temp = pdata[0] | (pdata[1] << 8);
		measured_data.count++;
	}
	else if(cluster_id == 0x001) {
		if(attrID == 0x20) {
			measured_data.battery_mv = pdata[0] * 100;
			measured_data.count++;
		}
		else if(attrID == 0x21) {
			measured_data.battery_level = pdata[0] >> 1;
			measured_data.count++;
		}
	}
}
...
static void sampleGW_zclReadRspCmd(zclIncoming_t *pInMsg)
{
	...
	*pBuf++ = pReadRspCmd->numAttr;
	...
	tst_msg_adv(pInMsg->msg->indInfo.cluster_id, pReadRspCmd->attrList[0].attrID, pReadRspCmd->attrList[0].data);
...
//    printf("sampleGW_zclReportCmd\n");
	...
	*pBuf++ = pReportCmd->numAttr;
	...
	tst_msg_adv(pInMsg->msg->indInfo.cluster_id, pReportCmd->attrList[0].attrID, pReportCmd->attrList[0].attrData);
